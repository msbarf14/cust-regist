...
>
    <a
        ...
        @endif
        @class([
            'relative flex items-center justify-center gap-x-3 rounded-lg px-2 py-2 text-sm text-gray-400 outline-none transition duration-75 hover:bg-gray-100 focus:bg-gray-100 dark:text-gray-200 dark:hover:bg-white/5 dark:focus:bg-white/5',
            'font-semibold' => ! $grouped,
            'font-medium' => $grouped,
            'bg-gray-100/0 text-primary-600 dark:bg-white/5 dark:text-primary-400' => $active,
        ])
    >
        @if (filled($icon))
            <x-filament::icon
